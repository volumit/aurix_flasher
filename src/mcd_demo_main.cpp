...
mcd_return_et mcdd_write_block(mcd_core_st *core, const mcd_addr_st *addr, uint32_t num_bytes);
...
int do_tests(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg);
int do_rsthlt(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg);
int do_frequency(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg);
int do_status(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg, int *status);
int do_write_mem(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg, unsigned int addr, unsigned int data);
int do_read_mem(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg, unsigned int addr, unsigned int *data);
int do_run(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg);
mcd_return_et mcdd_write_block(mcd_core_st *core, unsigned address, uint32_t num_bytes, unsigned char *pmem);
extern unsigned char* get_memory_buffer(unsigned int dst, unsigned int *paddr, unsigned int *plength);
extern int init_memory_buffers(void);
extern void load_file(char *filename);
extern int target_download_ram(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg, char *pname);
extern int target_download_init(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg);
extern int target_download_flash(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg, char *pname, int emul);
extern int target_download_flash_double(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg, char *pname, int emul);
...
int main(int argc, char** argv) 
{
  ...
  assert(server != NULL);
  target_download_init(server, core, &core_con_info_core, &core_ip_reg);
  char iflash[20];
  strcpy(&iflash[0],"flash_TC3xx.hex");
  target_download_ram(server, core, &core_con_info_core, &core_ip_reg,&iflash[0]);
  target_download_flash_double(server, core, &core_con_info_core, &core_ip_reg, argv[1], 0);
  //  ret = do_tests(server, core, &core_con_info_core, &core_ip_reg);
...
int do_rsthlt(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg)
{
  ...
  mcdd_handle_err(stdout, &core, ret);
  printf("IP 0x%8.8X State %-8s \n", core_ip, mcdt_get_core_state_string(state.state));
  return (0);
}
...
int do_frequency(mcd_server_st *server, mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg)
{
  ...
  ret = mcdd_set_acc_hw_frequency(server, 30000000);
...
int do_tests(mcd_server_st *server,mcd_core_st *core, mcd_core_con_info_st *core_con_info_core, mcd_register_info_st *core_ip_reg)
{
  ...
  printf("Write N bytes:        writebl <addr> <n_bytes>\n");
  ...
  //gets_s(line);  // Not clear why this is needed here to empty the input buffer?
  ...
  while (true) {
	...
	continue; // Try to reconnect core in the beginning of the while loop
      ...
      else if (strncmp(cmd, "writebl", 8) == 0) {
	  txDemo.addr.address = param0;
	  if (n_items == 3) {
	      ret = mcdd_write_block(core, &txDemo.addr, param1);
	      mcdd_handle_err(stdout, &core, ret);
	  }
      }
...
mcd_return_et mcdd_read_block(mcd_core_st *core, const mcd_addr_st *addr, uint32_t num_bytes)
{
  ...
  printf("Read %d byte\n", num_bytes - numBytesRemaining);
  {
    uint32_t cnt;
    uint32_t i;
    cnt = num_bytes - numBytesRemaining;
    printf("Block Read %d \n", cnt);
    for (i=0; i<cnt; i+=1)
      {
	printf("%2.2x ", data[i]);
...
mcd_return_et mcdd_write_block(mcd_core_st *core, unsigned address, uint32_t num_bytes, unsigned char *pmem)
{
  ...
  uint8_t *data = new uint8_t[num_bytes];
  uint32_t i;
  for (i = 0; i < num_bytes; i += 1) data[i] = pmem[i];
  mcd_tx_st tx;
  ...
  memset(&tx, 0, sizeof(tx));  // Set all to default values
  //	memcpy(&tx.addr, addr, sizeof(mcd_addr_st));
  tx.addr.address = address;
  tx.access_type = MCD_TX_AT_W;
  tx.data = data;
...
mcd_return_et mcdd_write_block(mcd_core_st *core, const mcd_addr_st *addr, uint32_t num_bytes)
{
  ...
  uint8_t *data = new uint8_t[num_bytes];
  uint32_t i;
  for (i = 0; i < num_bytes; i += 1) data[i] = i;
  mcd_tx_st tx;
