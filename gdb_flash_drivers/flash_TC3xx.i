# 1 "flash_TC3xx.c"
# 1 "E:\\AURIX_MINGW\\SRC\\aurix_flasher\\gdb_flash_drivers//"
# 1 "<built-in>"
#define __STDC__ 1
...
#define __VERSION__ "4.9.4 build on 2021-03-13"
...
#define HF_STATUS 0xF8040010u
#define HF_CLRE 0xF8040038u
#define HF_ERRSR 0xF8040034u
...
int flashing(unsigned long * parameter, char **notused)
{
  ...
  (*((volatile unsigned int *)(0xF8040038u)))=0x3e;
  if (parameter[4] == 1)
    {
      parameter[4]=0x100;
      ...
      burst=0x0;
      if ((parameter[0] & 0x8F000000)==0x80000000) burst=0x40;
      if ((parameter[0] & 0x8F000000)==0x8F000000) burst=0x8;
      if (burst==0x0) { parameter[4] = 0x100;__asm ("	debug"); }
      while (parameter[3] > 0)
 {
   for (i=0; i<burst; i++)
     {
   ...
   if (burst==0x40) { (*((volatile unsigned int *)(0xAF005554))) = 0x50; }
   if (burst==0x8) { (*((volatile unsigned int *)(0xAF005554))) = 0x5D; }
   i=0;
   while (i<burst)
     {
       ...
       i+=1;
       (*((volatile unsigned int *)(0xAF0055F0)))=low32bit;
       __asm ("	dsync");
       (*((volatile unsigned int *)(0xAF0055F4)))=high32bit;
       __asm ("	dsync");
     }

   (*((volatile unsigned int *)(0xAF00AA50))) = addr_aligned;
   __asm ("	dsync");
   (*((volatile unsigned int *)(0xAF00AA58))) = 0x0;
   __asm ("	dsync");
   (*((volatile unsigned int *)(0xAF00AAA8))) = 0xa0;
   __asm ("	dsync");
   (*((volatile unsigned int *)(0xAF00AAA8))) = 0xa6;
   __asm ("	dsync");

   for (timeout=0x4000; timeout>0; timeout--)
     {
       read = (*((volatile unsigned int *)(0xF8040010u)));
       __asm ("	dsync");
       if ((read & 0x00FF) ==0) { timeout=0; (*((volatile unsigned int *)(0xAF005554))) = 0xFA; }
       if (timeout==1) {
    parameter[2] = addr_aligned; parameter[4] = 0x0100; (*((volatile unsigned int *)(0xAF005554))) = 0xFA; __asm ("	debug");
       }
     }
   addr_aligned+=burst*4;
 }
      if (((*((volatile unsigned int *)(0xF8040034u))) & 0x003E)!=0)
 parameter[4] = 0x0100; else parameter[4]=0x0;
      (*((volatile unsigned int *)(0xAF005554))) = 0xFA;
      __asm ("	debug");
    }
  ...
  if (parameter[4] == 2)
    {
      parameter[4]=0x100;
      (*((volatile unsigned int *)(0xAF00AA50))) = parameter[2];
      __asm ("	dsync");
      (*((volatile unsigned int *)(0xAF00AA58))) = 1;
      __asm ("	dsync");
      (*((volatile unsigned int *)(0xAF00AAA8))) = 0x80;
      __asm ("	dsync");
      (*((volatile unsigned int *)(0xAF00AAA8))) = 0x50;
      __asm ("	dsync");
      while (1==1)
 {
   ...
   read = (*((volatile unsigned int *)(0xF8040010u)));
   __asm ("	dsync");
   if (((*((volatile unsigned int *)(0xF8040010u))) & 0x00FF) ==0)
     {
       if (((*((volatile unsigned int *)(0xF8040034u))) & 0x003E)!=0)
  parameter[4] = 0x0100; else parameter[4]=0x0;
       __asm ("	debug");
 ...
 {
   i=0;
   (*((volatile unsigned int *)(0xAF005554))) = 0x5D;
   while (i<burst)
     {
       low32bit=data[j+i];
       (*((volatile unsigned int *)(0xAF0055F0)))=low32bit;
   ...
   (*((volatile unsigned int *)(0xAF00AA50))) = parameter[2] + (j<<2);
   __asm ("	dsync");
   (*((volatile unsigned int *)(0xAF00AA58))) = 0x0;
   __asm ("	dsync");
   (*((volatile unsigned int *)(0xAF00AAA8))) = 0xa0;
   ...
   for (timeout=0x4000; timeout>0; timeout--)
     {
       ...
       if ((read & 0x00FF) ==0) { timeout=0; (*((volatile unsigned int *)(0xAF005554))) = 0xFA; }
       if (timeout==1) {
    parameter[2] = addr_aligned; parameter[4] = 0x0102; (*((volatile unsigned int *)(0xAF005554))) = 0xFA;
    __asm ("	debug");
       }
     }
   j+=burst;
